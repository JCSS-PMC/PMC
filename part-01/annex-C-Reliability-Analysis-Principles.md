...
This Annex C summarizes the principles and procedures used in formulating and solving risk related problems via reliability analysis. It is neither as broad nor as detailed as available textbooks on this subject, some of which are included in the bibliography. Its purpose is to underpin the updating and decision-making methodologies presented in part 2 of this document.
...
The limit state function, $g(\mathbf{s}, \mathbf{r})$, can often be separated into one resistance function, $r($.$) , and$ one loading (or action effect) function, $s(.)$ , in which case equation $(C.1)$ can be expressed as:

$$
r(\mathbf{r})-s(\mathbf{s})=0 \tag{C.2}
$$

### $\underline{\text{Structural Reliability}}$

Load, material and geometry parameters are subject to uncertainties, which can be classified according to their nature, see section 3. They can, thus, be represented by random variables (this being the simplest possible probabilistic representation, whereas more advanced models might be appropriate in certain situations, such as random fields). The variables $\mathbf{S}$ and $\mathbf{R}$ are often referred to as "basic random variables" (where the upper case letter is used for denoting random variables) and may be collectively represented by a random vector $\mathbf{X}$.

In this context, failure is a probabilistic event and its probability of occurrence, $P_{f}$, is given by:

$$
P_{f}=\text{Prob}~ \left\lbrace g(\mathbf{X}) \leq 0 \right\rbrace =\text{Prob}~ \left\lbrace \mathrm{M} \leq 0 \right\rbrace \tag{C.3a}
$$

where, $\mathrm{M}=g(\mathbf{X})$. Note that $\mathrm{M}$ is also a random variable, called the safety margin.
...
where $\mathrm{R}=r~(\mathbf{R})$ and $\mathrm{S}=s~(\mathbf{S})$ are random variables associated with resistance and loading respectively. This expression is useful in the context of the discussion in section 2.2 on code formats and partial safety factors but will not be further used herein.

The failure probability defined in eqn $(A.5a)$ can also be expressed as follows:

$$
P_{f}=\int_{g(\mathbf{x}) \leq 0} f_{\mathbf{X}}(\mathbf{x}) \mathrm{d} \mathbf{x} \tag{C.3b}
$$
...
$$
\beta=-\Phi^{-1}(P_f)=\Phi^{-1}\left(P_{S}\right) \tag{C.5}
$$

where $\Phi^{-1}(.)$ is the inverse of the standard normal distribution function, see Table $A.1.$

**Table C.1: Relationship between $\boldsymbol{\beta}$ and $\boldsymbol{P}_{\boldsymbol{f}}$**

| $P_{f}$ | $10^{-1}$ | $10^{-2}$ | $10^{-3}$ | $10^{-4}$ | $10^{-5}$ | $10^{-6}$ | $10^{-7}$ |
| :-----: | :-------: | :-------: | :-------: | :-------: | :-------: | :-------: | :-------: |
| $\beta$ |    1.3    |    2.3    |    3.1    |    3.7    |    4.2    |    4.7    |    5.2    |

In most engineering applications, complete statistical information about the basic random variables $\mathbf{X}$ is not available and, furthermore, the function $g(.)$ is a mathematical model which idealizes the limit state. In this respect, the probability of failure evaluated from eqn $(C.3a)$ or $(C.3b)$ is a point estimate given a particular set of assumptions regarding probabilistic modelling and a particular mathematical model for $g(.)$. The uncertainties associated with these models can be represented in terms of a vector of random parameters $\mathbf{Q}$, and hence the limit state function may be re-written as $g(\mathbf{X}, \mathbf{Q})$. It is important to note that the nature of uncertainties represented by the basic random variables $\mathbf{X}$ and the parameters $\mathbf{Q}$ is different. Whereas uncetainties in $\mathbf{X}$ cannot be influenced without changing the physical characteristics of the problem (e.g. changing the steel grade), uncertainties in $\mathbf{Q}$ can be influenced by the use of alternative methods and collection of additional data.

In this context, eqn $(C.3b)$ may be recast as follows

$$
P_{f}(\boldsymbol{\theta})=\int_{g(\mathbf{x}, \theta) \leq 0} f_{\mathbf{X} \mid \Theta}~(\mathbf{x} \mid \boldsymbol{\theta}) \mathrm{d} \mathbf{x} \tag{C.6}
$$
...
$$
\overline{P_{f}}=\mathrm{E}\left[P_{f}(\boldsymbol{\theta})\right]=\int_{\theta} P_{f}(\boldsymbol{\theta}) f_{\Theta}(\boldsymbol{\theta}) \mathrm{d} \boldsymbol{\theta} \tag{C.7a}
$$

where $f_{\theta}(\theta)$ is the joint probability density function of $\theta$. The corresponding reliability index is given by

$$
\bar{\beta}=-\Phi^{-1}\left(\overline{P_{f}}\right) \tag{C.7b}
$$
...
There are two fundamental systems, see $Fig. C.1$:
...
$$
\begin{align}
&P_{f, sys}=\mathrm{P} \left[\mathrm{E_1} \cup \mathrm{E_2} \cup \ldots \cup \mathrm{E_n} \right] \quad \text{for a series system} \tag{C.8a} \\
&P_{f, sys}=\mathrm{P} \left[\mathrm{E_1} \cap \mathrm{E_2} \cap \ldots \cap \mathrm{E_n} \right] \quad \text{for a parallel system} \tag{C.8b}
\end{align}
$$
...
More general systems can be constructed by combining the two fundamental types. It is fair to say that system methods are more developed for skeletal rather than continuous structures. Important results from system reliability theory are summarized in section 4.
...
$$
\mathbf{T}: \quad \mathbf{X}=\left(X_{1}, X_{2}, \ldots X_{n}\right) \quad \rightarrow \quad \mathbf{U}=\left(U_{1}, U_{2}, \ldots U_{n}\right) \tag{C.9}
$$

where $U_{1}, U_{2}, \ldots U_{n}$ are independent standard normal variables (i.e. with zero mean value and unit standard deviation). Hence, the basic variable space (including the limit state function) is transformed into a standard normal space, see Fig. $C.5$. The special properties of the standard normal space lead to several important results, as discussed below.
...
This scalar quantity is known as the Hasofer-Lind reliability index, $\beta_{\mathrm{HL}}$, i.e.

$$
\beta_{H L}=\left(\sum_{i=1}^{n} u_{i}^{* 2}\right)^{1 / 2} \tag{C.10}
$$

Note that $\mathbf{u}^{*}$ can also be written as

$$
\mathbf{u}^{*}=\beta_{\mathrm{HL}}~ \mathbf{\alpha} \tag{C.11a}
$$
...
$$
\sum_{i=1}^{n} \alpha_{1}^{2}=1 \tag{C.11b}
$$
...
$$
P_{f \mathrm{FORM}}=\Phi\left(-\beta_{\mathrm{HL}}\right) \tag{C.12a}
$$
...
$$
P_{f S O R M}=\Phi\left(-\beta_{H L}\right) \prod_{j=1}^{n-1}\left(1-\beta_{H L}~ \kappa_{j}\right)^{-1 / 2} \tag{C.12b}
$$
...
$$
P_{f s y s}=\mathrm{P}\left[\bigcup_{j=1}^{\mathrm{m}} \mathrm{F}_{j}\right] \tag{C.13}
$$

where, $\mathrm{F}_{j}$ is the event corresponding to the failure of the $j$ th component. By describing this event in terms of a safety margin $\mathrm{M}_{j}$

$$
P\left[F_{j}\right]=P\left[M_{j} \leq 0\right] \approx \Phi\left(-\beta_{j}\right) \tag{C.14}
$$

where $\beta_{j}$ is its corresponding FORM reliability index, it can be shown that in a first-order approximation

$$
P_{f s y s}=1-\Phi_{\mathrm{m}}[\widetilde{\beta} ; \widetilde{\widetilde{\rho}}] \tag{C.15a}
$$

where $\Phi_{\mathrm{m}}[.]$ is the multi-variate standard normal distribution function, $\widetilde{\boldsymbol{\beta}}$ is the $(m \times 1)$ vector of component reliability indices and $\widetilde{\widetilde{\rho}}{ }$ is the $(m \times m)$ correlation matrix between safety margins with elements given by

$$
\rho_{j k}=\sum_{i=1}^{n} \alpha_{i j} \alpha_{i k} \quad, j, k=1,2, \ldots, \mathrm{m} \tag{C.15b}
$$

where $\alpha_{i j}$ is the sensitivity factor corresponding to the $i$ th random variable in the $j$ th margin. In some cases, especially when the number of components becomes large, evaluation of equation $(C.15)$ becomes cumbersome and bounds to the system failure probability may prove sufficient.

Simple first-order linear bounds are given by

$$
\overset{m}{\underset{j=1}{Max}}\left[P\left(F_{j}\right)\right] \leq P_{f s y s} \leq Min \left[\left(\sum_{j=1}^{m} P\left(F_{j}\right)\right), 1\right] \tag{C.16a}
$$
...
$$
P_{\text {fsys }}=P\left[\overset{m}{\underset{j=1}{\cap}}\left(F_{j}\right)\right]=P\left[\overset{m}{\underset{j=1}{\cap}}\left(M_{j} \leq 0\right)\right] \tag{C.17}
$$

and the corresponding first-order approximation is

$$
P_{f sys}=\Phi_{\mathrm{m}}[-\widetilde{\beta} ; \widetilde{\widetilde{\rho}}] \tag{C.18}
$$

Simple bounds are given by

$$
0 \leq P_{f s y s} \leq \overset{m}{\underset{j=1}{Min}}\left[P\left(F_{j}\right)\right] \tag{C.19a}
$$

These are usually too wide for practical applications. An improved upper bound is

$$
P_{f sys} \leq \overset{m}{\underset{j, k=1}{Min}}\left[P\left(F_{j} \cap F_{k}\right)\right] \tag{C.19b}
$$

The error involved in the first-order evaluation of the intersections, $\mathrm{P}\left[\mathrm{F}_{j} \cap \mathrm{F}_{k}\right]$, is, to a large extent, influenced by the non-linearity of the margins at their respective design points. In order to obtain a better estimate of the intersection probabilities, an improvement on the selection of linearisation points has been suggested.
...
Thus, the elementary reliability problem described through equations $(C.3a)$ and $(C.3 b)$ may now be formulated as:

$$
P_{f}(t)=Prob~\left\lbrace\mathbf{R}(t) \leq \boldsymbol{S}(t)\right\rbrace=Prob~\left\lbrace g(\mathbf{X}(t)) \leq 0 ~00,0 \right\rbrace \tag{C.20a}
$$

where $g(\mathbf{X}(t))=\mathrm{M}(t)$ is a time-dependent safety margin, and

$$
P_{f}(t)=\int_{\mathrm{g}(\mathbf{x}(\mathrm{t})) \leq 0} f_{\mathbf{X}(t)}~(\mathbf{x}(t))~\mathrm{d} \mathbf{x}(t) \tag{C.20b}
$$

is the instantaneous failure probability at time $t$, assuming that the structure was safe at time less than $t$.

In time-dependent reliability problems, interest often lies in estimating the probability of failure over a time interval, say from 0 to $t_{\mathrm{L}}$. This could be obtained by integrating $P_{f}(t)$ over the interval $\left[0, t_{\mathrm{L}}\right]$, bearing in mind the correlation characteristics in time of the process $\mathbf{X}(t)$ or, sometimes more conveniently, the process $\mathbf{R}(t)$, the process $\mathbf{S}(t)$, as well as any cross correlation between $\mathbf{R}(t)$ and $\mathbf{S}(t)$. Note that the load effect process $\mathbf{S}(t)$ is often composed of additive components, $\mathrm{S}_{1}(t), \mathrm{S}_{2}(\mathrm{t}), \ldots$, for each of which the time fluctuations may have different features (e.g. continuous variation, pulse-type variation, spikes).

Interest may also lie in predicting when $\mathrm{S}(t)$ crosses $\mathrm{R}(t)$ for the first time, see Figure $C.8$, or the probability that such an event would occur within a specified time interval. These considerations give rise to so-called 'crossing' problems, which are treated using stochastic process theory. A key concept for such problems is the rate at which a random process $\mathrm{X}(t)$ 'upcrosses' (or crosses with a positive slope) a barrier or level $\xi$, as shown in Figure $A.9$. This upcrossing rate is a function of the joint probability density function of the process and its derivative, and is given by Rice's formula

$$
v_{\xi}^{+}=\int_{0}^{\infty} \dot{x} f_{X \dot{X}}(\xi, \dot{x}) d \dot{x} \tag{C.21}
$$

where the rate in general represents an ensemble average at time $t$. For a number of common stochastic processes, useful results have been obtained starting from Equation $(C.21)$. An important simplification can be introduced if individual crossings can be treated as independent events and the occurences may be approximated by a Poisson distribution, which might be a reasonable assumption for certain rare load events.
...
The theory of stochastic load combination is used in situations where a structure is subjected to two or more time-varying actions acting simultaneously. When these actions are independent, perhaps the most important observation is that it is highly unlikely that each action will reach its peak lifetime value at the same moment in time. Thus, considering two time-varying load processes $p_{1}(t), p_{2}(t), 0 \leq t \leq \mathrm{T}$, acting simultaneously, for which their combined effect may be expressed as a linear combination $p_{1}(t)+p_{2}(t)$, the random variable of interest is:

$$
X=\text{max}_{\mathrm{T}}\left\lbrace p_{1}(t)+p_{2}(t)\right\rbrace \tag{C.22a}
$$

If the loads are independent, replacing $X$ by $\max _{\mathrm{T}}\left\{p_{1}(t)\right\}+\max _{\mathrm{T}}\left\{p_{2}(t)\right\}$ leads to very conservative results. However, the distribution of $X$ can be derived in few cases only. One possible way of dealing with this problem, which also leads to a relatively simple deterministic code format, is to replace $X$ with the following

$$
X' = \text{max}_T \left\lbrace
\begin{array}{l}
\max_T\left\lbrace p_1(t) \right\rbrace + p_2(t) \\ \\ \tag{C.22b}
...
The FBC (Ferry Borges-Castanheta) process is generated by a sequence of independent identically distributed random variables, each acting over a given (deterministic) time interval. This is shown in Fig. $C.11$ where the total reference period T is made up of $n_{i}$ repetitions where $n_{i}=\mathrm{T} / \tau_{i}$. Hence, the FBC process is a rectangular pulse process with changes in amplitude occurring at equal intervals. Because of independence, the maximum value in the reference period $\mathrm{T}$ is given by

$$
\mathrm{F}_{\max _{\mathrm{T}} \mathrm{X}_{\mathrm{i}}}\left(x_{i}\right)=\left[\mathrm{F}_{\mathrm{X}_{\mathrm{i}}}\left(x_{i}\right)\right]^{n_{i}} \tag{C.23}
$$
...
The first-passage probability, $P_{f}(t)$ during a period $\left[0, t_{\mathrm{L}}\right]$ is

$$
P_{f}\left(t_{\mathrm{L}}\right)=1-\mathrm{P}\left[N\left(t_{\mathrm{L}}\right)=0 \mid \mathbf{X}(0) \in D\right] P[\mathbf{X}(0) \in D] \tag{C.24a}
$$

where $\mathbf{X}(0) \in D$ signifies that the process $\mathbf{X}(t)$ starts in the safe domain and $N\left(t_{\mathrm{L}}\right)$ is the number of outcrossings in the interval $\left[0, t_{\mathrm{L}}\right]$. The second probability term is equivalent to $1-P_{f}(0)$, where $P_{f}(0)$ is the probability of failure at $t=0$. Equation $(C.28a)$ can be re-written as

$$
P_{f}(t_\mathrm{L})=P_{f}(0)+\left(1-P_{f}(0)\right)(1-\mathrm{P}[N(t_\mathrm{L})=0]) \tag{C.24b}
$$

from which different approximations may be derived depending on the relative magnitude of the terms. A useful bound is

$$
P_{f}\left(t_{\mathrm{L}}\right) \leq P_{f}(0)+\mathrm{E}\left[N\left(t_{\mathrm{L}}\right)\right] \tag{C.25}
$$

where the first term may be calculated by FORM/SORM and the expected number of outcrossings, $\mathrm{E}\left[N\left(t_{\mathrm{L}}\right)\right]$, is calculated by Rice's formula or one of its generalisations. Alternatively, parallel system concepts can be employed.

## Figures

![Schematic representation of series and parallel systems](images/Figure-C.1.jpg)
Figure C.1: Schematic representation of series and parallel systems

![Idealised load-displacement response of structural elements](images/Figure-C.2.jpg) 
Figure C.2: Idealised load-displacement response of structural elements

![Effect of element correlation and system size on failure probability (a) series system (b) parallel system](images/Figure-C.3.jpg)
Figure C.3: Effect of element correlation and system size on failure probability
(a) series system
(b) parallel system

![Schematic representations (a) random process (b) random field](images/Figure-C.4.jpg)
Figure C.4: Schematic representations
(a) random process
(b) random field

![Limit state surface in basic variable and standard normal space](images/Figure-C.5.jpg)
Figure C.5: Limit state surface in basic variable and standard normal space

![Failure region as union of component failure events for series system](images/Figure-C.6(a).jpg)
Figure C.6(a): Failure region as union of component failure events for series system

![Failure region as intersection of component failure events for parallel system](images/Figure-C.6(b).jpg)
Figure C.6(b): Failure region as intersection of component failure events for parallel system

![General time-dependent reliability problem](images/Figure-C.7.jpg)
Figure C.7: General time-dependent reliability problem

![Schematic representation of crossing problems (a) slowly varying resistance (b) rapidly varying resistance](images/Figure-C.8.jpg)
Figure C.8: Schematic representation of crossing problems 
(a) slowly varying resistance 
(b) rapidly varying resistance

![Fundamental barrier crossing problem](images/Figure-C.9.jpg)
Figure C.9: Fundamental barrier crossing problem

![Damage accumulation problem](images/Figure-C.10.jpg)
Figure C.10: Damage accumulation problem

![Realization of an FBC process](images/Figure-C.11.jpg)
Figure C.11: Realization of an FBC process

![Time-dependent safety margin and schematic representation of vector outcrossing](images/Figure-C.12.jpg)
